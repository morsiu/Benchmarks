using System;
using Impl = LibraryInterfacePerformance.GenericsAndInterfacesRefRangesDefault.Library;

namespace LibraryInterfacePerformance.GenericsAndInterfacesRefRangesDefault.Consumer
{
        ...
        where T : IComparable<T>
    {
        ...
        public bool IntersectsWith(Range<T> other) =>
            Impl.RangeOperations.IntersectsWith<T, Range<T>>(ref this, ref other);

        public Range<T> Intersect(Range<T> other)
        {
            var result = default(Range<T>);
            return Impl.RangeOperations.Intersect<T, Range<T>, Ranges<T>>(ref this, ref other, ref result)
                ? result
                : new Range<T>();
        }
